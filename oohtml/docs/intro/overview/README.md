...
OOHTML is a set of features that make common UI development paradigms possible as native web platform features. These features may be used individually or together for some great UI-authoring capabilites. Here is an overview:
...
*HTML Imports* are a declarative way to use the *exports* of an *HTML Module* everywhere in the main document.
...
Also, *state objects* are *live objects* that can be observed for changes using the [Observer API](../../spec/the-observer-api).
